...
void do_bgfg(char **argv) 
{
    ...
    int bg = 0; // Checks if the command is fg or bg
    if (strstr(argv[0], "bg") != NULL) {
        bg = 1;
    }

    // Check formatting of arguments
    if (job_str[0] == '%' && isdigit(job_str[1])) {
        jid = atoi(job_str+1);
        job = getjobjid(jobs, jid);
        if (job == NULL) { // Check to make sure job exists
            printf("%%%d: No such job\n", jid);
            return;
        }
    }
    else if (job_str[0] != '%' && isdigit(job_str[0])) {
        pid = atoi(job_str);
        job = getjobpid(jobs, pid);
        if (job == NULL) { // Check to make sure process exists
    ...
    if (bg) {  // Take a job from the foreground and move it to the background

        job->state = BG;
        kill(-1*pid, SIGCONT);
        printf("[%d] (%d) %s\n", jid, pid, job->cmdline);
    }
    else { // Take a job from the background and move it to the foreground

        if (!pid) pid = job->pid; // If JID was provided in the command line, we need to assign PID here
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        struct job_t* update = getjobpid(jobs, pid);

        if (WIFSTOPPED(status)) {
            // Update the process' state to stopped
            update->state = ST;
            printf("Job [%d] (%d) stopped by signal 20\n", update->jid, pid); // This line is required by the checker
            fflush(stdout);
        }
        else if (WIFSIGNALED(status)) {
            // The process was told to finish by terminator
